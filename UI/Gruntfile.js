/*!
 * Prezo UI's Gruntfile
 * http://getbootstrap.com
 * Copyright 2014 Pixeo BVBA.
 * All rights reserved
 */
...
module.exports = function(grunt) {
	...
	grunt.initConfig({
		...
		banner: '/*!\n' +
			' * Prezo UI v<%= pkg.version %> (<%= pkg.homepage %>)\n' +
			' * Copyright <%= grunt.template.today("yyyy") %> <%= pkg.author %>\n' +
			' * All rights reserved\n' +
			' */\n',
		jqueryCheck: 'if (typeof jQuery === \'undefined\') { throw new Error(\'Prezo UI\\\'s JavaScript requires jQuery\') }\n\n',
		...
		less: {
			compileCore: {
				...
				files: {
					'dist/css/<%= pkg.name %>.css': 'less/prezo_ui.less'
				}
			},
			compileTheme: {
				options: {
		...
		csslint: {
			...
			src: [
				'dist/css/prezo_ui.css',
				'dist/css/prezo_ui-theme.css'
			],
			examples: [
				'docs/examples/**/*.css'
			],
		...
		jade: {
			compile: {
				...
				files: {
					...
					'prototype/_includes/customizer-variables.html': 'prototype/_jade/customizer-variables.jade',
					'prototype/_includes/nav/customize.html': 'prototype/_jade/customizer-nav.jade'
				}
			}
		},
		...
		watch: {
			...
			prototype: {
				files: ['less/**/*.less'],
				tasks: ['less:compileCore', 'autoprefixer:core', 'cssmin:core']
			}
		},
	...
	grunt.registerTask('dist-prototype', 'copy:prototype');
